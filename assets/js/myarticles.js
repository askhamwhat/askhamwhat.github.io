...
function jsonarXivFeed(feed)
{
    ...
    else if (arxiv_format === 'plain') {
	makearXivplain(feed); }
    else if (arxiv_format === 'plainul') {
	makearXivplainul(feed); }
    else {
	    makearXiv(feed); }
}
...
function makearXivplain(feed)
{
    ...
    for (x=0; x<num_entries; x++) {
	//Add the numeral in brackets with a space
	html += '<dt> <div class="list-title">\n' + (x+1)+ '. ' +feed.entries[x].title	    //Add the title in a span
	    +' [';
	//add a span with the ref to the id in it
	    //now add the formats listed in the dictioary at feed.entries[x].formats
	    for (format_name in feed.entries[x].formats) {
    ...
    html += '<br /><span id="authorid_hook">[ This list is powered by an <a href="https://arxiv.org/a/'+arxiv_authorid + '">arXiv author id</a> and the <a href="https://arxiv.org/help/myarticles">myarticles</a> widget ]</span>';
    //close the arxiv container div
    html += '</dl>\n</div>\n'
    document.getElementById('arxivfeed').innerHTML=html;
}

function makearXivplainul(feed)
{
    var x = 0;
    // Remove style
    var html = '<div>\n';
    var format_name = '';
    //Everything is contained in a ul
    html += '<ul>\n';
    //Add each entry
    if (arxiv_max_entries === 0) {
	    num_entries = feed.entries.length;
    ...
    for (x=0; x<num_entries; x++) {
	//Add the numeral in brackets with a space
	html += '<li>' + feed.entries[x].authors + '. ' + feed.entries[x].title + '. ' ;  //Add the title
	//now add the formats listed in the dictioary at feed.entries[x].formats
	for (format_name in feed.entries[x].formats) {
	    if (feed.entries[x].formats.hasOwnProperty(format_name)) {
		var format_value = feed.entries[x].formats[format_name];
    ...
    html += '\n</ul>\n';
    html +='<br /><p>Showing the most recent '+num_entries+' of '+feed.entries.length+' total entries, all ' + feed.entries.length +' entries available at <a href="https://arxiv.org/a/' + arxiv_authorid+ '">arXiv.org</a>.\n';
